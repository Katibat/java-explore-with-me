...
class CommentAdminServiceTest {
    ...
    @Test
    public void deleteCommentByIdTestOk() {
    ...
    @Test
    public void deleteAllCommentsByEventTestOk() {
        int commentCount = 5;
        Event event = generateAndPersistEvent();
        List<Comment> comments = generateAndPersistCommentsForEvent(commentCount, event);
        assertEquals(commentCount, comments.size());
        service.deleteAllCommentsByEvent(event.getId());
        List<Comment> actual = service.findAllCommentsByEventId(event.getId());
        assertEquals(0, actual.size());
    }

    @Test
    public void getAllCommentsByEventIdTestOk() {
        int commentCount = 5;
        Event event = generateAndPersistEvent();
        List<Comment> comments = generateAndPersistCommentsForEvent(commentCount, event);
        assertEquals(commentCount, comments.size());
